...
>
> **cEOS limitation:** Queueing and scheduling (tx-queue priority, DWRR/bandwidth shares) and ingress DSCP rewrite are **forwarding-ASIC data-plane features** (Arad/Jericho). cEOS is a container with **no forwarding ASIC**, so this config is **accepted but not enforced** in the data plane. On cEOS the marking/scheduling will *not* protect voice under load, and the receiver will *not* see rewritten ToS bytes. Treat this lab as a **configuration / syntax exercise** verified with `show qos interface` and `show policy-map interface`; the latency-protection result is observable only on production hardware. See the Verification section for exactly what you will and won't see.
>
