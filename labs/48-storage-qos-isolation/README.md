...
>
> **cEOS limitation (verified live on 4.35.4M — stronger than "not enforced"):** ingress policing (`policy-map type qos` / `police`) and egress `tx-queue` bandwidth scheduling are **dataplane/ASIC** features. cEOS has no forwarding ASIC and **rejects** these commands outright — `policy-map type qos` and `tx-queue` return `% Invalid input` / `% Unavailable command (not supported on this hardware platform)` — so the config **does not even load** in the container, let alone enforce. This is **hardware-only** (DCS-7050X / 7280R / 7500R): study the config pattern and the trust-boundary model here, then apply and verify on real hardware. The throughput numbers in Verification describe production hardware. Same class as lab 42 (QoS) and lab 47 (DCB). The solution keeps the full production config as reference.
>
