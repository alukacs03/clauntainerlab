...
> **Note (cEOS limitation — read first):** DHCP snooping, Dynamic ARP Inspection, and IP Source Guard are all **hardware/ASIC-enforced** in EOS — the binding table is programmed into the forwarding chip, and ARP/IP packets are trapped or dropped there. cEOS is a container with **no forwarding ASIC**, so on this lab the commands are *config-accepted but not (fully) enforced*: the rogue DHCP server is **not** actually blocked, spoofed ARPs are **not** actually dropped, and spoofed source IPs **still pass**. This is the same class of limitation flagged in labs 05/06/47 (storm-control, port-security, PFC/ETS). Treat this lab as: **configure the trifecta correctly, then confirm operational state with `show` commands** — not "watch packets get dropped." On real hardware (or a physical Arista lab) the drops happen as described. Where a verification step would only enforce on hardware, it is marked **(HW only)** below.
