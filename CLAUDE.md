...
- **Flag untested platform-specific syntax explicitly.** Labs that touch EVPN, MLAG, BGP unnumbered, or other config areas where syntax has changed across vendor/EOS versions should call out the platform version they were written for and warn that the learner should verify against vendor docs if they hit unexpected `% Invalid input` errors. Don't silently assert syntax correctness for things Claude hasn't actually deployed.
