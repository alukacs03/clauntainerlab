...
| 45 | [vpn-on-mikrotik](labs/45-vpn-on-mikrotik) | WireGuard + IPsec site-to-site (MikroTik) | Ready | — | ⚠️ n/a |
...
- **⚠️ n/a** — requires a non-cEOS NOS image not on the Arista-focused test VM (lab 45 needs a MikroTik **CHR** image via vrnetlab). A confirmed interface-mapping fix (`ether1` is mgmt-reserved → data links on `ether2`+) was applied but **not** live-verified.
- **—** — not deployed in that pass (no live evidence yet; the config may still be fine). *(Currently none — every lab has been live-deployed at least once.)*
...
| ~~56~~ | ~~Hitless upgrade / rolling EOS upgrade~~ | Leaf-pair drain/upgrade/undrain dance (demo uses VARP + ECMP; same technique for MLAG/EVPN-MH pairs) |
...
### Chapter 11 — Advanced Routing (CCIE-depth track)

A bonus/specialist track that adds vendor-neutral CCIE-level routing depth on the **same Arista platform** — not a Cisco cert path, just the deeper knowledge. Built and **live-validated on cEOS** so each lab actually deploys and converges.
...
| — | _Multicast (PIM/IGMP)_ — partial | IGMP snooping works on cEOS; PIM/multicast-routing is hardware-only (`ip multicast-routing` is `% Unavailable` on cEOS — study lab) |
